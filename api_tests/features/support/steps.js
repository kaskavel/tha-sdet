...
const replaceEnvVariables = (text) => {
  // Regular expression to match variables like {{VAR_NAME}}
  const variableRegex = /{{\s*([^}\s]+)\s*}}/g;

  // Replace each occurrence of a variable with its value
  return text.replace(variableRegex, (_, envVariable) => {
    const envValue = process.env[envVariable];
    return envValue !== undefined ? envValue : `{{${envVariable}}}`;
  });
};

Given(/^I set json body to (.*)$/, function (json) {
  spec.withJson(json);
});

Given(/I set body to/, function (body) {
  try {
    const processedBody = replaceEnvVariables(body);
    spec.withJson(JSON.parse(processedBody));
  } catch(error) {
    spec.withBody(body);
  }
});
